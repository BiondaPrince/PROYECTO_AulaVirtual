...
export default function CourseTeacher(){
  ...
  const handleAnnouncementFormSubmit = (e) => {
    ...
    const existing = JSON.parse(localStorage.getItem(key) || '[]')
    const announcement = { id: Date.now(), title, message, date: new Date().toISOString() }
  ...
  const toggleThread = (threadId) => {
    setExpandedThreads(prev => prev.includes(threadId) ? prev.filter(id=>id!==threadId) : [...prev, threadId])
  }
  ...
  const submitComment = (threadId) => {
    ...
    if(idx === -1) return
    const post = { id: Date.now(), author: 'Docente', message: text, date: new Date().toISOString() }
    ...
    <div className="course-teacher-page">
      ...
      <div className="teacher-content">
            ...
            <form onSubmit={handleCreate} className="task-form">
              ...
              <div className="d-flex gap-2">
                <label className="form-group w-33">
                  Semana
                  <select className="form-control" value={weekNumber} onChange={e=>setWeekNumber(e.target.value)}>
                    {Array.from({length:18},(_,i)=>i+1).map(n=> <option key={n} value={n}>{n}</option>)}
                  </select>
                </label>
                <label className="form-group w-33">
                  Fecha de creación
                  <input type="datetime-local" className="form-control" value={creationDate} onChange={e=>setCreationDate(e.target.value)} />
                </label>
                <label className="form-group w-33">
                  Fecha y hora de cierre
                  <input type="datetime-local" className="form-control" value={dueDateTime} onChange={e=>setDueDateTime(e.target.value)} />
                </label>
              </div>
                  ...
                  <div className="card-body d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      <p className="muted mb-1">{t.description}</p>
                        <small className="muted">Creada: {t.creationDate ? new Date(t.creationDate).toLocaleString() : '—'}</small>
                        <br />
                        <small className="muted">Vence: {t.dueDateTime ? new Date(t.dueDateTime).toLocaleString() : (t.dueDate || 'No establecido')}</small>
                    </div>
                    <div className="d-flex gap-2">
                      <a className="btn btn-outline-secondary" href={`/course/${courseId}/task/${t.id}`}>Ver detalles</a>
                      <a className="btn btn-outline-primary" href={`/course/${courseId}#tasks`}>Ver como alumno</a>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {activeTab === 'view' && (
          <section className="teacher-section">
            <h3>Ver tareas</h3>
            <p className="muted">Listado público de tareas que verán los alumnos. Usa "Ir al curso" para navegar como alumno.</p>
            <div className="teacher-tasks-list">
              {tasks.length === 0 && <p>No hay tareas publicadas.</p>}
              {tasks.map(t => (
                <div key={t.id} className="task-card card d-flex align-items-center justify-content-between mb-3">
        ...
        {activeTab === 'forum' && (
          <section className="teacher-section">
            ...
            <p>Sección para que los alumnos realicen consultas y el docente responda.</p>
            <div className="d-flex justify-content-between align-items-center mb-2">
              <div className="muted">Listado de foros del foro.</div>
              <div>
                {!showForumForm && (
                  <button className="btn btn-sm btn-outline-success" onClick={() => setShowForumForm(true)}>Crear foro</button>
                )}
              </div>
            </div>
            ...
            {showForumForm && (
              <form onSubmit={handleForumFormSubmit} className="mb-3">
                <input className="form-control mb-1" placeholder="Título del foro" value={forumFormTitle} onChange={e=>setForumFormTitle(e.target.value)} />
                <textarea className="form-control mb-1" placeholder="Mensaje inicial" value={forumFormMessage} onChange={e=>setForumFormMessage(e.target.value)} />
                <div className="d-flex gap-2 mt-1">
                  <button className="btn btn-sm btn-success" type="submit">Crear foro</button>
                  <button type="button" className="btn btn-sm btn-outline-secondary" onClick={() => { setShowForumForm(false); setForumFormTitle(''); setForumFormMessage('') }}>Cancelar</button>
                </div>
              </form>
            )}
                  ...
                  <div className="card-body">
                    <div className="d-flex justify-content-between align-items-start">
                      <div>
                        ...
                        <small className="muted">Creado: {h.date ? new Date(h.date).toLocaleString() : ''}</small>
                      </div>
                      <div>
                        <button className="btn btn-sm btn-outline-primary" onClick={() => toggleThread(h.id)}>{expandedThreads.includes(h.id) ? 'Cerrar' : 'Ver foro'}</button>
                      </div>
                    </div>
                    ...
                    {expandedThreads.includes(h.id) && (
                      <div className="mt-3 thread-detail">
                        <div className="posts-list mb-2">
                          {(h.posts || []).length === 0 && <div className="muted">No hay respuestas aún.</div>}
                          {(h.posts || []).map(p => (
                        ...
                        <div className="comment-form">
                          <textarea className="form-control mb-1" placeholder="Escribir respuesta..." value={commentInputs[h.id] || ''} onChange={e=>setCommentForThread(h.id, e.target.value)} />
                          <div className="d-flex gap-2">
                            <button className="btn btn-sm btn-primary" onClick={() => submitComment(h.id)}>Responder</button>
                            <button className="btn btn-sm btn-outline-secondary" onClick={() => { setCommentForThread(h.id, ''); setExpandedThreads(prev=>prev.filter(id=>id!==h.id)) }}>Cerrar</button>
                          </div>
                        </div>
                      </div>
                    )}
        ...
        {activeTab === 'announcements' && (
          <section className="teacher-section">
            <h3>Anuncios</h3>
            <p className="muted">Área para publicar anuncios del curso. (Plantilla simple por ahora.)</p>
            <div className="d-flex justify-content-between align-items-center mb-2">
              <div className="muted">Anuncios del curso.</div>
              <div>
                {!showAnnouncementForm && (
                  <button className="btn btn-sm btn-outline-warning" onClick={() => setShowAnnouncementForm(true)}>Crear anuncio</button>
                )}
              </div>
            </div>
            ...
            {showAnnouncementForm && (
              <form onSubmit={handleAnnouncementFormSubmit} className="mb-3">
                <input className="form-control mb-1" placeholder="Título del anuncio" value={announcementTitleInput} onChange={e=>setAnnouncementTitleInput(e.target.value)} />
                <textarea className="form-control mb-1" placeholder="Mensaje del anuncio" value={announcementMessageInput} onChange={e=>setAnnouncementMessageInput(e.target.value)} />
                <div className="d-flex gap-2 mt-1">
                  <button className="btn btn-sm btn-warning" type="submit">Publicar anuncio</button>
                  <button type="button" className="btn btn-sm btn-outline-secondary" onClick={() => { setShowAnnouncementForm(false); setAnnouncementTitleInput(''); setAnnouncementMessageInput('') }}>Cancelar</button>
                </div>
              </form>
            )}

            <div className="announcements-list">
              {announcements.length === 0 && <div className="card p-3"><p>No hay anuncios publicados.</p></div>}
              {announcements.map(a => (
                <div key={a.id} className="card mb-2">
                  <div className="card-body">
                    <h5>{a.title}</h5>
                    <p className="muted">{a.message}</p>
                    <small className="muted">Publicado: {a.date ? new Date(a.date).toLocaleString() : ''}</small>
                  </div>
                </div>
              ))}
            </div>
